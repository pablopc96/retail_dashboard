...
function populateFilters() {
  ...
  const selectP = document.getElementById('productFilter');
  ...
  const selectC = document.getElementById('channelFilter');
  ...
  const selectB = document.getElementById('branchFilter');
  branchSet.forEach(b=>{
    const opt = document.createElement('option');
    opt.value = b;
    opt.text = b;
...
function updateCards() {
  ...
  if(branchFilter !== 'all') filtered = filtered.filter(d=>d.branch===branchFilter);
  ...
  // avg price siempre 1 decimal
  document.getElementById('lastMonthAvgPrice').textContent = `$${lastAvgPrice.toFixed(1)}`;

  setDelta('lastMonthRevenueDelta', lastRevenue, prevRevenue);
  setDelta('lastMonthUnitsDelta', lastUnits, prevUnits);
  // avg price delta igual, pero solo triángulo
