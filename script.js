/* script.js - dashboard funcional revisado */
let salesData = [];
let chart = null;
...
function updateCards(){
  ...
  // calcular último mes
  const lastMonth = lastKeys.slice(-1)[0];
  const lastMonthData = filtered.filter(d => (d.date||'').slice(0,7)===lastMonth);
  const prevMonth = lastKeys.slice(-2,-1)[0];
  const prevMonthData = filtered.filter(d => (d.date||'').slice(0,7)===prevMonth);

  const lastRevenue = lastMonthData.reduce((s,d)=>s+(d.revenue||0),0);
  const lastUnits = lastMonthData.reduce((s,d)=>s+(d.units||0),0);
  const lastAvg = lastUnits ? lastRevenue/lastUnits : 0;

  const prevRevenue = prevMonthData.reduce((s,d)=>s+(d.revenue||0),0);
  const prevUnits = prevMonthData.reduce((s,d)=>s+(d.units||0),0);
  ...
  setValue('lastMonthAvgPrice', lastAvg, true, {forceExact:true, decimals:1});

  setDelta('lastMonthRevenueDelta', lastRevenue, prevRevenue);
  setDelta('lastMonthUnitsDelta', lastUnits, prevUnits);
  setDelta('lastMonthAvgPriceDelta', lastAvg, prevAvg);
...
function setDelta(id,current,prev){
  ...
  el.textContent = delta ? `${delta>0?'+':''}${delta.toFixed(1)}%` : '';
  el.className = `text-lg mt-1 ${delta>=0?'text-green-500':'text-red-500'}`;
}
